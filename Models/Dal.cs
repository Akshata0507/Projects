...
namespace SocialMediaProject.Models
{
    public class Dal
    {
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM Registraion WHERE Email = '"+registration.Email+"' AND Password = '"+registration.Password+"' ",connection);
            ...
            { 
                ...
                reg.Email = Convert.ToString(dt.Rows[0]["Eamil"]);
        ...
        public Response AddNews(News news, SqlConnection connection)
        {
            ...
            Response response = new Response();
            SqlCommand cmd = new SqlCommand("INSERT  NewsDeclare(Title,Content,Email,IsActive,CreatedOn) VALUES('"+ news.Title +"','"+ news.Content +"','"+ news.Email +"',1,GETDATE())", connection);
            connection.Open();
            int i = cmd.ExecuteNonQuery();
            connection.Close();
            if (i > 0)
        ...
        public Response NewsList(SqlConnection connection)
        {
            ...
            Response response = new Response();
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM NewsDeclare WHERE IsActive = 1", connection);            
        ...
        public Response AddArticle(Article article, SqlConnection connection)
        {
            ...
            Response response = new Response();
            SqlCommand cmd = new SqlCommand("INSERT  Article(Title,Content,Email,Image,IsActive,IsApproved) VALUES('" + article.Title + "','" + article.Content + "','" + article.Email + "', '" + article.Image + "',1,0)", connection);
            connection.Open();
            int i = cmd.ExecuteNonQuery();
            connection.Close();
            if (i > 0)
        ...
        public Response AddEvent(Events events, SqlConnection connection)
        {
            ...
            Response response = new Response();
            SqlCommand cmd = new SqlCommand("INSERT  Events(Title,Content,Email,IsActive,CreatedOn) VALUES('" + events.Title + "','" + events.Content + "','" + events.Email + "',1,GETDATE())", connection);
            connection.Open();
            int i = cmd.ExecuteNonQuery();
            connection.Close();
            if (i > 0)
